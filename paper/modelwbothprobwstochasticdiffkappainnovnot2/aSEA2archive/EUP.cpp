...
struct paramsgolden //mngolden
{
    ...
    double *valueUL1nextST,*valueUS1nextST, *valueUL0nextST,*valueUS0nextST, *valueVEnextST, *valueVEcnextST, *valueWW1nextST,*valueWW0nextST;
    // Search function
...
{
    double tempWW1val, tempWW1VEval, tempWW0val, tempUS1val, tempUS0val, tempUL1val, tempUL0val,tempUL1VEval, tempUS1VEval, searchUsEoutST;
...
double focWWwID(const double xval, void * params)
{
    ...
    double cons,xgrid,dxgrid,valfoc,tempWW0,tempUS0,tempWW1,tempUS1, tempWW1VE, tempUS1VE, tempVE, searcheffVE, testval1, testval2, testval3, testval4;
...
double focUwID(const double xval, void * params)
{
    ...
    double cons,xgrid,dxgrid,tempWW1, tempWW0, tempWW1VE, tempUS1, tempUS0, tempUL1, tempUL0, tempVE, tempUS1VE, tempUL1VE, valfoc, searcheffWW, searcheffVE, testval1, testval2, testval3, testval4;
...
double focVE(const double xval, void * params)
{
    ...
    double cons,xgrid,dxgrid,valfoc,tempWW0, tempUS0, tempUL0, tempWW1VE, tempUL1VE, testval1, testval2, testval3, testval4, searcheffWW, tempVE;
    ...
    #if SEA2 == 1
    paramsXX.diffValueNextfoc=max(0.000000000001, (betapar*((1-mupar)*(tempWW1VE-tempUL1VE) + mupar*(tempWW0 - tempUS0))));
    paramsXX.ygridparXX = ygridfoc;
    
    if((betapar*((1-mupar)*(tempWW1VE-tempUL1VE) + mupar*(tempWW0 - tempUS0))) <= -0.0000001) {
    ...
    #if SEA2 == 1
    valfoc=utilc(cons)+disutilityW(searcheffWW)+betapar*(piW(ygridfoc,searcheffWW)*(1.0 - mupar)*tempWW1VE + piW(ygridfoc, searcheffWW)*mupar*tempWW0 + mupar*(1 - piW(ygridfoc, searcheffWW))*tempUS0 + (1 - mupar)*(1-piW(ygridfoc, searcheffWW))*tempUL1VE);
    
    valfoc=-valfoc;
    
    if (ibug==1) {
        printf("focVE %20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\n","igridfoc","xval","ixgrid","dxgrid","cons","utilc(cons)","endoKfoc","vfRHS","valfoc");
        printf("focVE %20d\t%20.15f\t%20d\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\n",igridfoc,xval,ixgrid,dxgrid,cons,utilc(cons),endoKfoc,betapar*(piW(ygridfoc,searcheffWW)*(1.0 - mupar)*tempWW1VE + piW(ygridfoc, searcheffWW)*mupar*tempWW0 + mupar*(1 - piW(ygridfoc, searcheffWW))*tempUS0 + (1 - mupar)*(1-piW(ygridfoc, searcheffWW))*tempUL1VE),valfoc);
        getchar();
    }
    #endif
...
void VFI(double *valueWW0, double *valueWW1, double *valueUS0, double *valueUS1, double *valueUL0, double *valueUL1, double *valueVE, double *valueVEc, double *endoK,double *saveWW0, double *saveWW1, double *saveUS0, double *saveUS1, double *saveUL0, double *saveUL1, double *saveVE, double *searchEffortWWUS0, double *searchEffortWWUL0, double *searchEffortWWUS1, double *searchEffortVEUS1, double *searchEffortWWUL1, double *searchEffortVEUL1, double *searchEffortWWVE,double *searchEffortVEWW)
{
    ...
    // For entrepreneur
    double *valueVEnext, *endoKnext, valfnmaxVE, savemaxVE, amaxsaveVE,productionVE,testvalueVE, wealthVE,searchvalWWVE;
    ...
    // temp val
    double tempU,tempB, tempUS0, tempVE, tempVE2, tempUL0, tempWW0, tempUL1VE, tempWW1, tempUL1, tempUS1, tempWW1VE, tempUS1VE, tempWW1VE2, tempUL1VE2;
   ...
   struct searchfendoKYY paramsKK;
    ...
    while ((critereendoK > epsilonendoK) && iterK < maxiterendoK)
    {
                            ...
                            if (wealthVE<=testvalueVE || wealthVE <= 0.0) // actual saving is higher than wealth (not possible)
                                {
                                    ...
                                    {//wealth is so small that consumption is almost zero
                                        ...
                                        #if SEA2 == 1
                                        paramsYY.diffValueNextfoc = max(0.000000000001, (betapar*((1-mupar)*(tempWW1VE2-tempUL1VE2) + mupar*(tempWW0 - tempUS0))));
                                        #endif
                                        #if SEA2 == 0
        ...
        for(ygrid=0; ygrid<maxprod;ygrid++)
        {
            for(igrid=0;igrid<maxgrid;igrid++)
            {
                ...
                if ((fabs(valueWW1[inx(igrid, ygrid)]-valueWW1next[inx(igrid, ygrid)]))>=critereVF){jcount=inx(igrid, ygrid);ivaltype=5;valmax=valueWW1[inx(igrid, ygrid)];valmaxold=valueWW1next[inx(igrid, ygrid)];}
    ...
    fprintf(endokoutfile,"%5s\t%5s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\n","TYPE","PROD","ASSET","ValueVE","ValueVEc", "SaveVE","endoKVE", "SearchWWVE");
    ...
    for(egrid=0;egrid<maxfirmtype;egrid++) {
        for(int ygrid = 0; ygrid < maxprod ; ygrid++)
        {
            ...
            {
                #if BRC == 1
                fprintf(endokoutfile,"%5d\t%5d\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\n",ygrid,egrid, grid[igrid],valueVE[inxE(igrid,ygrid,egrid)],valueVEc[inxE(igrid,ygrid,egrid)],saveVE[inxE(igrid,ygrid,egrid)],endoK[inxKK(igrid,ygrid, egrid)],searchEffortWWVE[inxE(igrid,ygrid,egrid)]);
                #endif
...
#if indexPM == 1
void SIMULATION(double *valueWW0, double *valueWW1, double *valueUS0, double *valueUS1, double *valueUL0, double *valueUL1, double *valueVE, double *valueVEc, double *endoK,double *saveWW0, double *saveWW1, double *saveUS0, double *saveUS1, double *saveUL0, double *saveUL1, double *saveVE, double *searchEffortWWUS0, double *searchEffortWWUL0, double *searchEffortWWUS1, double *searchEffortVEUS1, double *searchEffortWWUL1, double *searchEffortVEUL1, double *searchEffortWWVE,double *searchEffortVEWW,double *totalassets, double *totallabor, double *optimaltax, double *generatedmoment, double *start_distVE, double *start_distWW1, double *start_distWW0, double *start_distUL0, double *start_distUL1, double *start_distUS1, double *start_distUS0, double *distVE, double *distWW1, double *distWW0, double *distUL0, double *distUL1, double *distUS1, double *distUS0, double *fracVEE, double *fracUU)
{
#endif

// Need to change function value
#if indexPM == 0
void SIMULATION(double *valueWW0, double *valueWW1, double *valueUS0, double *valueUS1, double *valueUL0, double *valueUL1, double *valueVE, double *valueVEc, double *endoK,double *saveWW0, double *saveWW1, double *saveUS0, double *saveUS1, double *saveUL0, double *saveUL1, double *saveVE, double *searchEffortWWUS0, double *searchEffortWWUL0, double *searchEffortWWUS1, double *searchEffortVEUS1, double *searchEffortWWUL1, double *searchEffortVEUL1, double *searchEffortWWVE,double *searchEffortVEWW,double *totalassets, double *totallabor, double *optimaltax, double *generatedmoment, double *start_distVE, double *start_distWW1, double *start_distWW0, double *start_distUL0, double *start_distUL1, double *start_distUS1, double *start_distUS0)
{
#endif

#if indexPM == 0
double *distVE, *distWW1, *distWW0, *distUL0, *distUL1, *distUS1, *distUS0;
#endif

    // Entrepreneur distribution
    double *distVEold, *distassetVE, *saveresVE, fracVEtoWW, fracVEtoUL, fracVE;
    int *isaveVE;
    double tempVEWW1_0, tempVEUS1_0, tempVEUL1_0, tempVEWW1_1, tempVEUS1_1, tempVEUL1_1;
    ...
    // Statistics
    double *distVEtoUL, *disttotal, *distasset, *distWWtoVE, *distVEtoWW, *distWWtoU, *distVEtoU, *distUtoVE, *distUtoWW, *fracVEpability, *fracWWpability, *fracUpability, *aggWWtoVE, *aggVEtoWW, *aggUtoVE, *aggUtoWW, *aggWWtoU, *aggVEtoU, *necessityUS, *necessityUL, *necessityWW, necessityWWtoVE, necessityUStoVE, necessityULtoVE, top001p, top01p, top05p, top10p, top20p,bot20p,bot40p,bot60p,bot80p, mworthVE,mworthWW,mworthUS,mworthUL,capitalVE,productionVE,totalcapital,corproduction,totalproduction, KY, GINIval, distWWearning[maxprod], GINIearn, welfareUS, welfareUL, welfareWW, welfareVE, avgfirmsize, welfaretot;
    ...
    distWWtoU = (double *) calloc((ifulldim), sizeof(double));
    distUtoVE = (double *) calloc((ifulldim), sizeof(double));
    distUtoWW = (double *) calloc((ifulldim), sizeof(double));
    ...
    aggWWtoU = (double *) calloc((maxprod), sizeof(double));
    aggVEtoU = (double *) calloc((maxprod), sizeof(double));
    aggUtoVE = (double *) calloc((maxprod), sizeof(double));
    aggUtoWW = (double *) calloc((maxprod), sizeof(double));
    fracWWpability = (double *) calloc((maxprod), sizeof(double));
    fracVEpability = (double *) calloc((maxprod), sizeof(double));
    fracUpability = (double *) calloc((maxprod), sizeof(double));
    ...
    for(ygrid = 0.0; ygrid < maxprod; ygrid++)
    {
        for(igrid=0;igrid<maxgrid;igrid++)
        {
            for(egrid=0;egrid<maxfirmtype;egrid++)
            {
                ...
                distVEtoUL[inxE(igrid, ygrid, egrid)]=0.0;
            }
    ...
    for(ygrid = 0.0; ygrid < maxprod; ygrid++)
    {
        for(igrid=0;igrid<maxgrid;igrid++)
        {
            ...
            distWWtoU[inx(isaveWW0[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1-xipar)*etapar[ygrid]*(1.0 - saveresWW0[inx(igrid, ygrid)])*distWW0old[inx(igrid, ygrid)];
            distWWtoU[inx(min((maxgrid-1),(isaveWW0[inx(igrid, ygrid)]+1)), k)] += mprod[ygrid][k]*(1-xipar)*etapar[ygrid]*(saveresWW0[inx(igrid, ygrid)])*distWW0old[inx(igrid, ygrid)];
            ...
            distWWtoU[inx(isaveWW0[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(xipar)*etapar[ygrid]*(1.0 - saveresWW0[inx(igrid, ygrid)])*distWW0old[inx(igrid, ygrid)];
            distWWtoU[inx(min((maxgrid-1),(isaveWW0[inx(igrid, ygrid)]+1)), k)] += mprod[ygrid][k]*(xipar)*etapar[ygrid]*(saveresWW0[inx(igrid, ygrid)])*distWW0old[inx(igrid, ygrid)];
            ...
            for(int e = 0; e < maxfirmtype; e++) {
                ...
                distUtoVE[inx(isaveWW1c[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*mgtrans[k][e]*(1 - zetapar)*(etapar[ygrid])*piE(searchEffortVEWW[inx(igrid, ygrid)])*(1.0-saveresWW1[inx(igrid, ygrid)])*distWW1old[inx(igrid, ygrid)]*(((tempVEWW1_0 >= (valueUS1[inx(isaveWW1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
                distUtoVE[inx(min((maxgrid-1),(isaveWW1c[inx(igrid, ygrid)]+1)), k)] += mprod[ygrid][k]*mgtrans[k][e]*(1 - zetapar)*(etapar[ygrid])*piE(searchEffortVEWW[inx(igrid, ygrid)])*(saveresWW1[inx(igrid, ygrid)])*distWW1old[inx(igrid, ygrid)]*(((tempVEWW1_1 >= (valueUS1[inx(min((maxgrid-1),(isaveWW1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
            ...
            distWWtoU[inx(isaveWW1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1 - zetapar)*(etapar[ygrid])*(1.0 - piE(searchEffortVEWW[inx(igrid, ygrid)]))*(1.0-saveresWW1[inx(igrid, ygrid)])*distWW1old[inx(igrid, ygrid)];
            distWWtoU[inx(min((maxgrid-1),(isaveWW1[inx(igrid, ygrid)]+1)), k)] += mprod[ygrid][k]*(1 - zetapar)*(etapar[ygrid])*(1.0 - piE(searchEffortVEWW[inx(igrid, ygrid)]))*(saveresWW1[inx(igrid, ygrid)])*distWW1old[inx(igrid, ygrid)];
            ...
            distWWtoU[inx(isaveWW1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1 - zetapar)*(etapar[ygrid])*piE(searchEffortVEWW[inx(igrid, ygrid)])*(1.0-saveresWW1[inx(igrid, ygrid)])*distWW1old[inx(igrid, ygrid)]*(((tempVEWW1_0 < (valueUS1[inx(isaveWW1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
            distWWtoU[inx(min((maxgrid-1),(isaveWW1[inx(igrid, ygrid)]+1)), k)] += mprod[ygrid][k]*(1 - zetapar)*(etapar[ygrid])*piE(searchEffortVEWW[inx(igrid, ygrid)])*(saveresWW1[inx(igrid, ygrid)])*distWW1old[inx(igrid, ygrid)]*(((tempVEWW1_1 < (valueUS1[inx(min((maxgrid-1),(isaveWW1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
            ...
            distWWtoU[inx(isaveWW1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(zetapar)*(etapar[ygrid])*(1.0-saveresWW1[inx(igrid, ygrid)])*distWW1old[inx(igrid, ygrid)];
            distWWtoU[inx(min((maxgrid-1),(isaveWW1[inx(igrid, ygrid)]+1)), k)] += mprod[ygrid][k]*(zetapar)*(etapar[ygrid])*(saveresWW1[inx(igrid, ygrid)])*distWW1old[inx(igrid, ygrid)];
            ...
            distUtoWW[inx(isaveUS0[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1-xipar)*(piWU(ygrid, searchEffortWWUS0[inx(igrid,ygrid)]))*(1.0 - saveresUS0[inx(igrid, ygrid)])*distUS0old[inx(igrid, ygrid)];
            distUtoWW[inx(min((maxgrid-1),(isaveUS0[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(1-xipar)*(piWU(ygrid, searchEffortWWUS0[inx(igrid,ygrid)]))*(saveresUS0[inx(igrid, ygrid)])*distUS0old[inx(igrid, ygrid)];
            ...
            distUtoWW[inx(isaveUS0[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(xipar)*(piWU(ygrid, searchEffortWWUS0[inx(igrid,ygrid)]))*(1.0 - saveresUS0[inx(igrid, ygrid)])*distUS0old[inx(igrid, ygrid)];
            distUtoWW[inx(min((maxgrid-1),(isaveUS0[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(xipar)*(piWU(ygrid, searchEffortWWUS0[inx(igrid,ygrid)]))*(saveresUS0[inx(igrid, ygrid)])*distUS0old[inx(igrid, ygrid)];
            ...
            distUtoWW[inx(isaveUL0[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1-xipar)*(piWU(ygrid, searchEffortWWUL0[inx(igrid,ygrid)]))*(1.0 - saveresUL0[inx(igrid, ygrid)])*distUL0old[inx(igrid, ygrid)];
            distUtoWW[inx(min((maxgrid-1),(isaveUL0[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(1-xipar)*(piWU(ygrid, searchEffortWWUL0[inx(igrid,ygrid)]))*(saveresUL0[inx(igrid, ygrid)])*distUL0old[inx(igrid, ygrid)];
            ...
            distUtoWW[inx(isaveUL0[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(xipar)*(piWU(ygrid, searchEffortWWUL0[inx(igrid,ygrid)]))*(1.0 - saveresUL0[inx(igrid, ygrid)])*distUL0old[inx(igrid, ygrid)];
            distUtoWW[inx(min((maxgrid-1),(isaveUL0[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(xipar)*(piWU(ygrid, searchEffortWWUL0[inx(igrid,ygrid)]))*(saveresUL0[inx(igrid, ygrid)])*distUL0old[inx(igrid, ygrid)];
            ...
            distUtoWW[inx(isaveUS1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(zetapar)*(piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(1.0 - saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)];
            distUtoWW[inx(min((maxgrid-1),(isaveUS1[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(zetapar)*(piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)];
            ...
            distUtoWW[inx(isaveUS1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(1.0 - saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)]*(((tempVEUS1_0 <= (valueWW1[inx(isaveUS1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
            distUtoWW[inx(min((maxgrid-1),(isaveUS1[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)]*(((tempVEUS1_1 <= (valueWW1[inx(min((maxgrid-1),(isaveUS1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
            ...
            distUtoWW[inx(isaveUS1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(1.0 - piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(1.0 - saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)];
            distUtoWW[inx(min((maxgrid-1),(isaveUS1[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(1.0 - piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)];
            ...
            for(int e = 0; e < maxfirmtype; e++) {
                ...
                distUtoVE[inx(isaveUS1c[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*mgtrans[k][e]*(1.0 - zetapar)*(1.0 - pLTpar)*(piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(1.0 - saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)]*(((tempVEUS1_0 > (valueWW1[inx(isaveUS1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
                distUtoVE[inx(min((maxgrid-1),(isaveUS1c[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*mgtrans[k][e]*(1.0 - zetapar)*(1.0 - pLTpar)*(piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)]*(((tempVEUS1_1 > (valueWW1[inx(min((maxgrid-1),(isaveUS1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
                ...
                distUtoVE[inx(isaveUS1c[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*mgtrans[k][e]*(1 - pLTpar)*(1.0 - zetapar)*(1.0 - piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(1.0 - saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)]*(((tempVEUS1_0 >= (valueUS1[inx(isaveUS1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
                distUtoVE[inx(min((maxgrid-1),(isaveUS1c[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*mgtrans[k][e]*(1 - pLTpar)*(1.0 - zetapar)*(1.0 - piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)]*(((tempVEUS1_1 >= (valueUS1[inx(min((maxgrid-1),(isaveUS1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
                ...
                distUtoVE[inx(isaveUS1c[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*mgtrans[k][e]*(pLTpar)*(1.0 - zetapar)*(1.0 - piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(1.0 - saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)]*(((tempVEUS1_0 >= (valueUL1[inx(isaveUS1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
                distUtoVE[inx(min((maxgrid-1),(isaveUS1c[inx(igrid, ygrid)]+1)), k)] += mprod[ygrid][k]*mgtrans[k][e]*(pLTpar)*(1.0 - zetapar)*(1.0 - piWU(ygrid, searchEffortWWUS1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUS1[inx(igrid,ygrid)]))*(saveresUS1[inx(igrid, ygrid)])*distUS1old[inx(igrid, ygrid)]*(((tempVEUS1_1 >= (valueUL1[inx(min((maxgrid-1),(isaveUS1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
            ...
            distUtoWW[inx(isaveUL1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(zetapar)*(piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(1.0 - saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)];
            distUtoWW[inx(min((maxgrid-1),(isaveUL1[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(zetapar)*(piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)];
            ...
            distUtoWW[inx(isaveUL1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUL1[inx(igrid,ygrid)]))*(1.0 - saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)]*(((tempVEUL1_0 <= (valueWW1[inx(isaveUL1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
            distUtoWW[inx(min((maxgrid-1),(isaveUL1[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUL1[inx(igrid,ygrid)]))*(saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)]*(((tempVEUL1_1 <= (valueWW1[inx(min((maxgrid-1),(isaveUL1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
            ...
            distUtoWW[inx(isaveUL1[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(1.0 - piEU(searchEffortVEUL1[inx(igrid,ygrid)]))*(1.0 - saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)];
            distUtoWW[inx(min((maxgrid-1),(isaveUL1[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(1.0 - piEU(searchEffortVEUL1[inx(igrid,ygrid)]))*(saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)];
            ...
            for(int e = 0; e < maxfirmtype; e++) {
                ...
                distUtoVE[inx(isaveUL1c[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*mgtrans[k][e]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUL1[inx(igrid,ygrid)]))*(1.0 - saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)]*(((tempVEUL1_0 > (valueWW1[inx(isaveUL1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
                distUtoVE[inx(min((maxgrid-1),(isaveUL1c[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*mgtrans[k][e]*(1.0 - zetapar)*(piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUL1[inx(igrid,ygrid)]))*(saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)]*(((tempVEUL1_1 > (valueWW1[inx(min((maxgrid-1),(isaveUL1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
                ...
                distUtoVE[inx(isaveUL1c[inx(igrid, ygrid)], k)] += mprod[ygrid][k]*mgtrans[k][e]*(1.0 - zetapar)*(1.0 - piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUL1[inx(igrid,ygrid)]))*(1.0 - saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)]*(((tempVEUL1_0 >= (valueUL1[inx(isaveUL1[inx(igrid, ygrid)], k)])))?(1.0):(0.0));
                distUtoVE[inx(min((maxgrid-1),(isaveUL1c[inx(igrid, ygrid)]+1)), k)] +=  mprod[ygrid][k]*mgtrans[k][e]*(1.0 - zetapar)*(1.0 - piWU(ygrid, searchEffortWWUL1[inx(igrid,ygrid)]))*(piEU(searchEffortVEUL1[inx(igrid,ygrid)]))*(saveresUL1[inx(igrid, ygrid)])*distUL1old[inx(igrid, ygrid)]*(((tempVEUL1_1 >= (valueUL1[inx(min((maxgrid-1),(isaveUL1[inx(igrid, ygrid)]+1)), k)])))?(1.0):(0.0));
            ...
            for(egrid = 0; egrid < maxfirmtype; egrid++)
            {
                ...
                for(int e = 0; e < maxfirmtype; e++) {
                    ...
                    #if SEA2 == 1
                    distUS0[inx(isaveVE[inxE(igrid, ygrid, egrid)], k)] += mprod[ygrid][k]*mggtrans[ygrid][e]*(mupar)*(1.0 - piW(ygrid, searchEffortWWVE[inxE(igrid,ygrid,egrid)]))*(1.0 - saveresVE[inxE(igrid, ygrid, egrid)])*distVEold[inxE(igrid, ygrid, egrid)];
                    distUS0[inx(min((maxgrid-1),(isaveVE[inxE(igrid, ygrid, egrid)]+1)), k)] += mprod[ygrid][k]*mggtrans[ygrid][e]*(mupar)*(1.0 - piW(ygrid, searchEffortWWVE[inxE(igrid,ygrid,egrid)]))*(saveresVE[inxE(igrid, ygrid, egrid)])*distVEold[inxE(igrid, ygrid, egrid)];
                ...
                if(egrid == 0) {
                    ...
                    #if SEA2 == 1
                    distUS0[inx(isaveVE[inxE(igrid, ygrid, egrid)], k)] += mprod[ygrid][k]*(mupar)*(1.0 - piW(ygrid, searchEffortWWVE[inxE(igrid,ygrid,egrid)]))*(1.0 - saveresVE[inxE(igrid, ygrid, egrid)])*distVEold[inxE(igrid, ygrid, egrid)];
                    distUS0[inx(min((maxgrid-1),(isaveVE[inxE(igrid, ygrid, egrid)]+1)), k)] += mprod[ygrid][k]*(mupar)*(1.0 - piW(ygrid, searchEffortWWVE[inxE(igrid,ygrid,egrid)]))*(saveresVE[inxE(igrid, ygrid, egrid)])*distVEold[inxE(igrid, ygrid, egrid)];
...
//    
    ...
    fprintf(distoutfile,"%5s\t%5s\t%5s\t%20s\n","TYPE","PROD","ASSET","VE");
    ...
    for(ygrid=0;ygrid<maxprod;ygrid++)
    {
        for(igrid=0;igrid<maxgrid;igrid++)
        {
            for(egrid=0; egrid<maxfirmtype; egrid++)
            {
                fprintf(distoutfile,"%5d\t%5d\t%5d\t%20.15f\n",egrid, ygrid,igrid, distVE[inxE(igrid, ygrid, egrid)]);
            }
        }
    }
    ...
    distoutfile=fopen(distfirmsize, "w");
    setbuf ( distoutfile , NULL );
    
    
    fprintf(distoutfile,"%5s\t%5s\t%5s\t%20s\t%20s\n","TYPE","PROD","ASSET","endoK","VE");
    ...
    for(ygrid=0;ygrid<maxprod;ygrid++)
    {
        for(igrid=0;igrid<maxgrid;igrid++)
        {
            for(egrid=0; egrid<maxfirmtype; egrid++)
            {
                fprintf(distoutfile,"%5d\t%5d\t%5d\t%20.15f\t%20.15f\n",egrid, ygrid,igrid, endoK[inxE(igrid, ygrid, egrid)], distVE[inxE(igrid, ygrid, egrid)]);
            }
        }
    }
    ...
    *totallabor = 0.0;
    ...
    distassetUL[igrid]+=0.0;
        ...
        for(egrid=0;egrid<maxfirmtype; egrid++) {
            ...
            aggVEtoU[ygrid] += distVEtoUL[inxE(igrid, ygrid, egrid)];
        ...
        *totallabor+=distWW1[inx(igrid,ygrid)]*prod[ygrid];
        ...
        fracUpability[ygrid] += distUS1[inx(igrid, ygrid)] + distUS0[inx(igrid, ygrid)] +distUL1[inx(igrid, ygrid)] + distUL0[inx(igrid, ygrid)];
        aggUtoVE[ygrid] += distUtoVE[inx(igrid, ygrid)];
        aggUtoWW[ygrid] += distUtoWW[inx(igrid, ygrid)];
        aggWWtoU[ygrid] += distWWtoU[inx(igrid, ygrid)];
...
*fracUU = fracUL + fracUS;
    ...
    for(igrid = 0; igrid < maxgrid; igrid++) {
        for(egrid = 0; egrid < maxfirmtype; egrid++) {
            ENT1 += distVE[inxE(igrid,0,egrid)];
            ENT2 += distVE[inxE(igrid,1,egrid)];
            ENT3 += distVE[inxE(igrid,2,egrid)];
            
            SUM1 += distVE[inxE(igrid,0,egrid)];
            SUM2 += distVE[inxE(igrid,1,egrid)];
            SUM3 += distVE[inxE(igrid,2,egrid)];
        }
    ...
    for(igrid = 0; igrid < maxgrid; igrid++) {
        for(egrid = 0; egrid < maxfirmtype; egrid++) {
            AVGFIRM1 += distVE[inxE(igrid,0,egrid)]*endoK[inxE(igrid,0,egrid)];
            AVGFIRM2 += distVE[inxE(igrid,1,egrid)]*endoK[inxE(igrid,1,egrid)];
            AVGFIRM3 += distVE[inxE(igrid,2,egrid)]*endoK[inxE(igrid,2,egrid)];
            AVGASSET1 += distVE[inxE(igrid,0,egrid)]*grid[igrid];
            AVGASSET2 += distVE[inxE(igrid,1,egrid)]*grid[igrid];
            AVGASSET3 += distVE[inxE(igrid,2,egrid)]*grid[igrid];
        }
    }
...
//    fprintf(tempfileoutfile,"Nshare (US/US>E) %20.15f, \t Nshare(UL/UL>E): %20.15f, \t Nshare (U/U>E) : %20.15f\n",(necessityUStoVE)/(fracUStoVE), necessityULtoVE/(fracULtoVE), (necessityULtoVE + necessityUStoVE)/(fracUStoVE + fracULtoVE)); // 11th moment
    ...
    for(ygrid = 0.0; ygrid < maxprod; ygrid++)
    {
        fprintf(tempfileoutfile,"%20.15f\t",aggWWtoU[ygrid]/fracWWpability[ygrid]);
    }
        fprintf(tempfileoutfile,"\n");
    ...
    for(ygrid = 0.0; ygrid < maxprod; ygrid++)
    {
        fprintf(tempfileoutfile,"%20.15f\t",aggVEtoU[ygrid]/fracVEpability[ygrid]);
    }
        fprintf(tempfileoutfile,"\n");
    ...
    for(ygrid = 0.0; ygrid < maxprod; ygrid++)
    {
        fprintf(tempfileoutfile,"%20.15f\t",aggUtoWW[ygrid]/fracUpability[ygrid]);
    }
        fprintf(tempfileoutfile,"\n");
    ...
    for(ygrid = 0.0; ygrid < maxprod; ygrid++)
    {
        fprintf(tempfileoutfile,"%20.15f\t",aggUtoVE[ygrid]/fracUpability[ygrid]);
    }
        fprintf(tempfileoutfile,"\n");
    ...
    distoutfile=fopen(valueVE_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(valueWW1_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(valueWW0_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(valueUS1_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(valueUL1_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(valueUS0_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(valueUL0_0file, "w");
    setbuf ( distoutfile , NULL );
...
#if indexPMwrite == 2
    // DISTRIBUTION for projected methods:
    distoutfile=fopen(saveVE_Tfile, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(saveWW1_Tfile, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(saveWW0_Tfile, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(saveUS1_Tfile, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(saveUL1_Tfile, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(saveUS0_Tfile, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(saveUL0_Tfile, "w");
    setbuf ( distoutfile , NULL );
...
#if indexPMwrite == 1
    // DISTRIBUTION for projected methods:
    distoutfile=fopen(distVE_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(distWW1_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(distWW0_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(distUS1_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(distUL1_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(distUS0_0file, "w");
    setbuf ( distoutfile , NULL );
    ...
    distoutfile=fopen(distUL0_0file, "w");
    setbuf ( distoutfile , NULL );
...
startingprice[0] = 0.062596094282829;        endprice[0] = 0.062329798757569;
startingprice[1] = 1.173091866791511;        endprice[1] = 1.174527619082393;
startingprice[2] = 0.011306767524860;        endprice[2] = 0.011822296121589;
startingprice[3] = 4.535904370356310;        endprice[3] = 4.543620970938563;
startingprice[4] = 0.842732180951385;        endprice[4] = 0.841302111375331;
startingprice[5] = 0.108965440248076;        endprice[5] = 0.110313680722795;
startingprice[6] = 0.053159327880998;        endprice[6] = 0.053181369723295;
...
// CEV :: WELFARE IMPROVEMENT TAKING INTO ACCOUNT THE TRANSITION COST
double temp_EVWW0, temp_EVWW1, temp_EVUS0, temp_EVUL0, temp_EVUS1, temp_EVUL1, temp_CONSEVWW0, temp_CONSEVWW1, temp_CONSEVUS0, temp_CONSEVUL0, temp_CONSEVUS1, temp_CONSEVUL1, temp_EVVE, temp_CONSEVVE, *EVVE_T, *EVWW0_T, *EVWW1_T, *EVUS0_T, *EVUL0_T, *EVUS1_T, *EVUL1_T;
double temp_EVWW0_SS, temp_EVWW1_SS, temp_EVUS0_SS, temp_EVUL0_SS, temp_EVUS1_SS, temp_EVUL1_SS, temp_CONSEVWW0_SS, temp_CONSEVWW1_SS, temp_CONSEVUS0_SS, temp_CONSEVUL0_SS, temp_CONSEVUS1_SS,temp_CONSEVUL1_SS , *EVWW0_SS, *EVWW1_SS, *EVUS0_SS, *EVUL0_SS, *EVUS1_SS, *EVUL1_SS, temp_EVVE_SS,temp_CONSEVVE_SS, *EVVE_SS;

EVVE_T = (double *) calloc((ifulldimE), sizeof(double));
EVWW0_T = (double *) calloc((ifulldim), sizeof(double));
EVWW1_T = (double *) calloc((ifulldim), sizeof(double));
EVUS0_T = (double *) calloc((ifulldim), sizeof(double));
EVUL0_T = (double *) calloc((ifulldim), sizeof(double));
EVUS1_T = (double *) calloc((ifulldim), sizeof(double));
EVUL1_T = (double *) calloc((ifulldim), sizeof(double));

EVVE_SS = (double *) calloc((ifulldimE), sizeof(double));
EVWW0_SS = (double *) calloc((ifulldim), sizeof(double));
EVWW1_SS = (double *) calloc((ifulldim), sizeof(double));
EVUS0_SS = (double *) calloc((ifulldim), sizeof(double));
EVUL0_SS = (double *) calloc((ifulldim), sizeof(double));
EVUS1_SS = (double *) calloc((ifulldim), sizeof(double));
EVUL1_SS = (double *) calloc((ifulldim), sizeof(double));
    
/* IMPORTATION PART */
// FIRST PERIOD VFI (FOR CEV)
double *valueVE0, *valueWW10, *valueWW00, *valueUS10, *valueUL10, *valueUS00, *valueUL00, *valueVEc0;
valueVE0 = (double *) calloc((ifulldimE), sizeof(double));
valueVEc0 = (double *) calloc((ifulldimE), sizeof(double));
valueWW10 = (double *) calloc((ifulldim), sizeof(double));
valueWW00 = (double *) calloc((ifulldim), sizeof(double));
valueUL10 = (double *) calloc((ifulldim), sizeof(double));
valueUL00 = (double *) calloc((ifulldim), sizeof(double));
valueUS10 = (double *) calloc((ifulldim), sizeof(double));
valueUS00 = (double *) calloc((ifulldim), sizeof(double));

readinput(valueVE0, ifulldimE, "project_method/valueVE_0.out");
readinput(valueVEc0, ifulldimE, "project_method/valueVE_0.out");
readinput(valueWW10, ifulldim, "project_method/valueWW1_0.out");
readinput(valueWW00, ifulldim, "project_method/valueWW0_0.out");
readinput(valueUL10, ifulldim, "project_method/valueUL1_0.out");
readinput(valueUL00, ifulldim, "project_method/valueUL0_0.out");
readinput(valueUS10, ifulldim, "project_method/valueUS1_0.out");
readinput(valueUS00, ifulldim, "project_method/valueUS0_0.out");

// LAST PERIOD SAVING (FOR CEV)
double *saveVE_T, *saveWW0_T, *saveWW1_T, *saveUL0_T, *saveUS0_T, *saveUS1_T, *saveUL1_T;
saveVE_T = (double *) calloc((ifulldimE), sizeof(double));
saveWW0_T = (double *) calloc((ifulldim), sizeof(double));
saveWW1_T = (double *) calloc((ifulldim), sizeof(double));
saveUL0_T = (double *) calloc((ifulldim), sizeof(double));
saveUS0_T = (double *) calloc((ifulldim), sizeof(double));
saveUS1_T = (double *) calloc((ifulldim), sizeof(double));
saveUL1_T = (double *) calloc((ifulldim), sizeof(double));

readinput(saveVE_T, ifulldimE, "project_method/saveVE_Tfile.out");
readinput(saveWW0_T, ifulldim, "project_method/saveWW0_Tfile.out");
readinput(saveWW1_T, ifulldim, "project_method/saveWW1_Tfile.out");
readinput(saveUL0_T, ifulldim, "project_method/saveUL0_Tfile.out");
readinput(saveUS0_T, ifulldim, "project_method/saveUS0_Tfile.out");
readinput(saveUS1_T, ifulldim, "project_method/saveUS1_Tfile.out");
readinput(saveUL1_T, ifulldim, "project_method/saveUL1_Tfile.out");
...
//            printf("%f %f %f", relaxTP, pricepath[inxPrice(ii,jj)], nowpricepath[inxPrice(ii,jj)]);getchar();
    ...
    fprintf(pricefile,"%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\n","R", "W", "T", "K", "L", "E", "U");
    ...
    double temp_VALA, temp_VALB,temp_VALA_T, temp_VALB_T, VEILESS, VEILT[T-1];
    ...
    for(int ygrid = 0; ygrid < maxprod ; ygrid++)
    {
        ...
        temp_VALA += distWW1_0[inx(igrid,ygrid)]*valueWW10[inx(igrid,ygrid)];
        temp_VALA += distWW0_0[inx(igrid,ygrid)]*valueWW00[inx(igrid,ygrid)];
        temp_VALA += distUS1_0[inx(igrid,ygrid)]*valueUS10[inx(igrid,ygrid)];
        temp_VALA += distUS0_0[inx(igrid,ygrid)]*valueUS00[inx(igrid,ygrid)];
        temp_VALA += distUL1_0[inx(igrid,ygrid)]*valueUL10[inx(igrid,ygrid)];
        temp_VALA += distUL0_0[inx(igrid,ygrid)]*valueUL00[inx(igrid,ygrid)];
        
        
        temp_VALB += distWW1_T[inx(igrid,ygrid)]*valueWW1T[inx(igrid,ygrid)];
        temp_VALB += distWW0_T[inx(igrid,ygrid)]*valueWW0T[inx(igrid,ygrid)];
        temp_VALB += distUS1_T[inx(igrid,ygrid)]*valueUS1T[inx(igrid,ygrid)];
        temp_VALB += distUS0_T[inx(igrid,ygrid)]*valueUS0T[inx(igrid,ygrid)];
        temp_VALB += distUL1_T[inx(igrid,ygrid)]*valueUL1T[inx(igrid,ygrid)];
        temp_VALB += distUL0_T[inx(igrid,ygrid)]*valueUL0T[inx(igrid,ygrid)];
        
        
        EVWW0_SS[inx(igrid,ygrid)] = pow((valueWW0T[inx(igrid,ygrid)]/valueWW00[inx(igrid,ygrid)]),(1/(1-sigmapar))) - 1;
        EVWW1_SS[inx(igrid,ygrid)] = pow((valueWW1T[inx(igrid,ygrid)]/valueWW10[inx(igrid,ygrid)]),(1/(1-sigmapar))) - 1;
        EVUS0_SS[inx(igrid,ygrid)] = pow((valueUS0T[inx(igrid,ygrid)]/valueUS00[inx(igrid,ygrid)]),(1/(1-sigmapar))) - 1;
        EVUL0_SS[inx(igrid,ygrid)] = pow((valueUL0T[inx(igrid,ygrid)]/valueUL00[inx(igrid,ygrid)]),(1/(1-sigmapar))) - 1;
        EVUS1_SS[inx(igrid,ygrid)] = pow((valueUS1T[inx(igrid,ygrid)]/valueUS10[inx(igrid,ygrid)]),(1/(1-sigmapar))) - 1;
        EVUL1_SS[inx(igrid,ygrid)] = pow((valueUL1T[inx(igrid,ygrid)]/valueUL10[inx(igrid,ygrid)]),(1/(1-sigmapar))) - 1;
        
        EVWW0_T[inx(igrid,ygrid)] = 0.0;
        EVWW1_T[inx(igrid,ygrid)] = 0.0;
        EVUS0_T[inx(igrid,ygrid)] = 0.0;
        EVUL0_T[inx(igrid,ygrid)] = 0.0;
        EVUS1_T[inx(igrid,ygrid)] = 0.0;
        EVUL1_T[inx(igrid,ygrid)] = 0.0;
        
            for(int egrid=0;egrid<maxfirmtype;egrid++) {
                temp_VALA += distVE_0[inxE(igrid,ygrid, egrid)]*valueVE0[inxE(igrid,ygrid, egrid)];
                temp_VALB += distVE_T[inxE(igrid,ygrid, egrid)]*valueVET[inxE(igrid,ygrid, egrid)];
                EVVE_SS[inxE(igrid,ygrid, egrid)] =  pow((valueVET[inxE(igrid,ygrid, egrid)]/valueVE0[inxE(igrid,ygrid, egrid)]),(1/(1-sigmapar))) - 1;
                
                
//                printf("%f %f %f %f %f %f", valueVET[inxE(igrid,ygrid, egrid)], valueVE0[inxE(igrid,ygrid, egrid)], temp_CONSEVVE_SS, temp_EVVE_SS, temp_EVVE_SS0, EVVE_SS[inxE(igrid,ygrid, egrid)]); getchar();
                
                EVVE_T[inxE(igrid,ygrid, egrid)] = 0.0;
            }
            
        }
    }

    // VEIL OF IGNORANCE WELFARE COMPARISON
    ...
    temp_VALB_T = 0.0;
        ...
        for(int ygrid = 0; ygrid < maxprod ; ygrid++)
        {
            ...
            temp_VALA_T += distWW1[inxTP((T-2-ii), igrid, ygrid)]*valueWW1[inxTP((T-2-ii), igrid, ygrid)];
            temp_VALA_T += distWW0[inxTP((T-2-ii), igrid, ygrid)]*valueWW0[inxTP((T-2-ii), igrid, ygrid)];
            temp_VALA_T += distUS1[inxTP((T-2-ii), igrid, ygrid)]*valueUS1[inxTP((T-2-ii), igrid, ygrid)];
            temp_VALA_T += distUS0[inxTP((T-2-ii), igrid, ygrid)]*valueUS0[inxTP((T-2-ii), igrid, ygrid)];
            temp_VALA_T += distUL1[inxTP((T-2-ii), igrid, ygrid)]*valueUL1[inxTP((T-2-ii), igrid, ygrid)];
            temp_VALA_T += distUL0[inxTP((T-2-ii), igrid, ygrid)]*valueUL0[inxTP((T-2-ii), igrid, ygrid)];
            
            
            temp_VALB_T += distWW1[inxTP((T-1-ii), igrid, ygrid)]*valueWW1[inxTP((T-1-ii), igrid, ygrid)];
            temp_VALB_T += distWW0[inxTP((T-1-ii), igrid, ygrid)]*valueWW0[inxTP((T-1-ii), igrid, ygrid)];
            temp_VALB_T += distUS1[inxTP((T-1-ii), igrid, ygrid)]*valueUS1[inxTP((T-1-ii), igrid, ygrid)];
            temp_VALB_T += distUS0[inxTP((T-1-ii), igrid, ygrid)]*valueUS0[inxTP((T-1-ii), igrid, ygrid)];
            temp_VALB_T += distUL1[inxTP((T-1-ii), igrid, ygrid)]*valueUL1[inxTP((T-1-ii), igrid, ygrid)];
            temp_VALB_T += distUL0[inxTP((T-1-ii), igrid, ygrid)]*valueUL0[inxTP((T-1-ii), igrid, ygrid)];
            
            EVWW0_T[inx(igrid,ygrid)] += pow((valueWW0[inxTP((T-2-ii), igrid, ygrid)]/valueWW0[inxTP((T-1-ii), igrid, ygrid)]),(1/(1-sigmapar))) - 1;
            EVWW1_T[inx(igrid,ygrid)] += pow((valueWW1[inxTP((T-2-ii), igrid, ygrid)]/valueWW1[inxTP((T-1-ii), igrid, ygrid)]),(1/(1-sigmapar))) - 1;
            EVUS0_T[inx(igrid,ygrid)] += pow((valueUS0[inxTP((T-2-ii), igrid, ygrid)]/valueUS0[inxTP((T-1-ii), igrid, ygrid)]),(1/(1-sigmapar))) - 1;
            EVUL0_T[inx(igrid,ygrid)] += pow((valueUL0[inxTP((T-2-ii), igrid, ygrid)]/valueUL0[inxTP((T-1-ii), igrid, ygrid)]),(1/(1-sigmapar))) - 1;
            EVUS1_T[inx(igrid,ygrid)] += pow((valueUS1[inxTP((T-2-ii), igrid, ygrid)]/valueUS1[inxTP((T-1-ii), igrid, ygrid)]),(1/(1-sigmapar))) - 1;
            EVUL1_T[inx(igrid,ygrid)] += pow((valueUL1[inxTP((T-2-ii), igrid, ygrid)]/valueUL1[inxTP((T-1-ii), igrid, ygrid)]),(1/(1-sigmapar))) - 1;
             
                for(int egrid=0;egrid<maxfirmtype;egrid++) {
                    temp_VALA_T += distVE[inxTP((T-2-ii), igrid, ygrid)]*valueVE[inxTPE((T-2-ii), igrid, ygrid, egrid)];
                    temp_VALB_T += distVE[inxTP((T-1-ii), igrid, ygrid)]*valueVE[inxTPE((T-1-ii), igrid, ygrid, egrid)];
                    EVVE_T[inxE(igrid,ygrid, egrid)] += pow((valueVE[inxTPE((T-2-ii), igrid, ygrid, egrid)]/valueVE[inxTPE((T-1-ii), igrid, ygrid, egrid)]),(1/(1-sigmapar))) - 1;
    ...
    printf("Crit cvng: %20.15f, %20.15f", critTP, VEILESS);
    
    
    pricefile=fopen(VEILTfile, "w");
    setbuf ( pricefile , NULL );
    
    
    fprintf(pricefile,"%20s\n","VEILT");
    
    for(int ygrid = 1; ygrid < (T-1) ; ygrid++)
    {
        fprintf(pricefile,"%20.15f\n",VEILT[ygrid]);
    }

    fclose(pricefile);
    
     
    pricefile=fopen(CEVall, "w");
    setbuf ( pricefile , NULL );
    
    
    fprintf(pricefile,"%5s\t%5s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\t%20s\n","ASSET", "PROD", "distWW0", "SSWW0", "TWW0","distWW1", "SSWW1", "TWW1","distUS0", "SSUS0", "TUS0","distUS1", "SSUS1", "TUS1","distUL0", "SSUL0", "TUL0","distUL1", "SSUL1", "TUL1");
    
    for(int ygrid = 0; ygrid < maxprod ; ygrid++)
    {
        for(int igrid=0;igrid<maxgrid;igrid++)
        {
        fprintf(pricefile,"%20.15f\t%5d\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\t%20.15f\n",grid[igrid], ygrid, distWW0_T[inx(igrid, ygrid)],EVWW0_SS[inx(igrid,ygrid)], EVWW0_T[inx(igrid,ygrid)],distWW1_T[inx(igrid, ygrid)],EVWW1_SS[inx(igrid,ygrid)], EVWW1_T[inx(igrid,ygrid)],distUS0_T[inx(igrid, ygrid)],EVUS0_SS[inx(igrid,ygrid)], EVUS0_T[inx(igrid,ygrid)],distUS1_T[inx(igrid, ygrid)],EVUS1_SS[inx(igrid,ygrid)], EVUS1_T[inx(igrid,ygrid)],distUL0_T[inx(igrid, ygrid)],EVUL0_SS[inx(igrid,ygrid)], EVUL0_T[inx(igrid,ygrid)],distUL1_T[inx(igrid, ygrid)],EVUL1_SS[inx(igrid,ygrid)], EVUL1_T[inx(igrid,ygrid)] );
    ...
    pricefile=fopen(CEVVE, "w");
    setbuf ( pricefile , NULL );
    
    
    fprintf(pricefile,"%20s\t%5s\t%5s\t%20s\t%20s\t%20s\n","ASSET","TYPE", "PROD", "distVE", "SSVE", "TVE");
    ...
    for(int ygrid = 0; ygrid < maxprod ; ygrid++)
    {
        ...
        {
            for(int egrid=0;egrid<maxfirmtype;egrid++) {
               fprintf(pricefile,"%20.15f\t%5d\t%5d\t%20.15f\t%20.15f\t%20.15f\n",grid[igrid], egrid, ygrid, distVE_T[inxE(igrid, ygrid,egrid)],EVVE_SS[inxE(igrid,ygrid,egrid)], EVVE_T[inxE(igrid,ygrid,egrid)]);
            }
...
double ComputeEquilibrium(double *PARAM)
{
    ...
    // Entrepreneur
    double *valueVE, *saveVE, *valueVEc, *searchEffortWWVE, *endoK;
    ...
    // STARTING DISTRIBUTION
    double *start_distVE, *start_distUS0, *start_distUS1, *start_distWW0, *start_distWW1, *start_distUL0, *start_distUL1;
    ...
    while(critprice > epsilonprice) // loop over r and w (and possibly u)
    {
        ...
        // VFI Computation
        VFI(valueWW0, valueWW1, valueUS0, valueUS1, valueUL0, valueUL1, valueVE, valueVEc, endoK, saveWW0, saveWW1, saveUS0, saveUS1, saveUL0, saveUL1, saveVE, searchEffortWWUS0, searchEffortWWUL0, searchEffortWWUS1, searchEffortVEUS1, searchEffortWWUL1, searchEffortVEUL1, searchEffortWWVE, searchEffortVEWW);
            
        printf("VFI ok\n");
        
        // SIMULATION
        #if indexPM == 0
        SIMULATION(valueWW0, valueWW1, valueUS0, valueUS1, valueUL0, valueUL1, valueVE, valueVEc, endoK, saveWW0, saveWW1, saveUS0, saveUS1, saveUL0, saveUL1, saveVE, searchEffortWWUS0, searchEffortWWUL0, searchEffortWWUS1, searchEffortVEUS1, searchEffortWWUL1, searchEffortVEUL1, searchEffortWWVE, searchEffortVEWW, &capitalout,&laborout,&taxout,genmoments, start_distVE, start_distWW1, start_distWW0, start_distUL0, start_distUL1, start_distUS1, start_distUS0);
        #endif
